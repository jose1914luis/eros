var rederizarCanvas = function (imagen, url, maxHeight, maxWidth, alturaDisplay, anchoDisplay) {
    ...
    img.onload = function () {
        ...
        rederizar($(imagen), alturaDisplay, anchoDisplay, height, width);
...
var rederizar = function (imagen, altura, ancho, alturaReal, anchuraReal) {

    var maxWidth = ancho; // Max width for the image
    var maxHeight = altura;    // Max height for the image
    var ratio = 0;  // Used for aspect ratio

    var width = anchuraReal;    // Current image width
    var height = alturaReal;  // Current image height
    // Check if the current width is larger than the max
    if (width > maxWidth) {
        ratio = maxWidth / width;   // get ratio for scaling image
